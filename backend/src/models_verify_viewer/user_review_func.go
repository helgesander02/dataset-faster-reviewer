...
func (pr *PendingReview) CreateBackup() error {
	timestamp := time.Now()
	filename := fmt.Sprintf("pending_review_%s.json", timestamp.Format("20060102_150405"))
	backupPath := filepath.Join(pr.backupDir, filename)
	...
	log.Printf("Created backup: %s with %d items", filename, len(pr.Items))

	if err := pr.cleanupOldBackups(); err != nil {
...
func (pr *PendingReview) cleanupOldBackups() error {
	files, err := os.ReadDir(pr.backupDir)
	if err != nil {
	...
	if len(backupFiles) > MaxBackupCount {
		for i := MaxBackupCount; i < len(backupFiles); i++ {
			oldBackupPath := filepath.Join(pr.backupDir, backupFiles[i].Name())
...
func (pr *PendingReview) ListBackups() ([]BackupInfo, error) {
	files, err := os.ReadDir(pr.backupDir)
	if err != nil {
	...
	for _, file := range files {
		if strings.HasPrefix(file.Name(), "pending_review_") && strings.HasSuffix(file.Name(), ".json") {
			itemCount := 0
			backupPath := filepath.Join(pr.backupDir, file.Name())
...
func (pr *PendingReview) RestoreFromBackup(filename string) (PendingReview, error) {
	backupPath := filepath.Join(pr.backupDir, filename)
...
func (pr *PendingReview) GetBackupPath() string {
	return pr.backupDir
}
