...
const PageItem = React.memo<ListChildComponentProps>(function PageItem({ index, style, data }) {
  const { 
    getPageData, 
    selectedImages, 
    handleImageClick,
  ...
  useEffect(() => {
    ...
    // Don't preload from every PageItem - only from selected page change
  }, [index, pageData, loadPage]);

  if (pageData.isLoading) {
    return (
      <div style={style}>
        <div className="h-full p-5 box-border border-b border-gray-200 bg-white overflow-y-auto flex flex-col">
  ...
  if (pageData.isEmpty) {
    return (
      <div style={style}>
        <div className="h-full p-5 box-border border-b border-gray-200 bg-white overflow-y-auto flex flex-col">
          <div className="flex items-center justify-center p-12 text-center text-gray-500 text-base font-medium">
